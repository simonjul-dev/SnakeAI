...
#11 inputs 1 hidden layer with 256 nodes, 3 outputs
...
class Linear_QNet(nn.Module):
    ...
    def forward(self, x): #this is a feed-forrward neural net
    ...
    def save(self, file_name='model.pth'): #saving the mdoel
        model_folder_path= './model'
...
class QTrainer:
    def __init__(self, model, lr, gamma): #initializig
        self.lr = lr
        self.gamma = gamma
        self.model = model
    ...
    def train_step(self, state, action, reward, next_state, done): #trainer
        ...
        if len(state.shape) == 1: #if there is 1 dimension
        ...
        pred = self.model(state) #using the Qmodelpredict equation above
